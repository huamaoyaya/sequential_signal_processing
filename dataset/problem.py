...
def prepare(problem_set, data_dir, max_length, batch_size, device, opt):
    if problem_set not in ['wmt32k', 'lm1b','fatigue_data']:
        raise Exception("only ['wmt32k', 'lm1b','fatigue_data'] problem set supported.")
    ...
    elif problem_set == 'fatigue':
        from dataset import fg
        fg.prepare(max_length, batch_size, device, opt, data_dir)
